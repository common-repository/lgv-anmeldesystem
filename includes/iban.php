 ...
 * See: https://github.com/cmpayments/iban/blob/master/src/lib/IBAN.php
 *
...
class LGV_AS_IBAN
{
    ...
    public function validate(&$error = null)
    {
        if (!$this->isCountryCodeValid()) {
            //$error = 'IBAN Ländercode wird nicht unterstützt';
			$error = 1;
        } elseif (!$this->isLengthValid()) {
            //$error = 'IBAN Länge ist ungültig';
			$error = 2;
        } elseif (!$this->isFormatValid()) {
            //$error = 'IBAN Format ist ungültig';
			$error = 3;
        } elseif (!$this->isChecksumValid()) {
            //$error = 'IBAN Prüfsumme ist ungültig (Zahlendreher?)';
			$error = 4;
        } else {
            //$error = '';
			$error = 0;
